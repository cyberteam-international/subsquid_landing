...
export default function Indexer({}: Props) {
    ...
    const setTabs = () => {
        return indexerData.diagrams.map((item, index) => {
            return (
              <div key={index}
                   className={classNames(style.indexer__tabs__item, activeTab === index && style.indexer__tabs__item_active)}
                   onClick={() => setActiveTab(index)}>
                  <p>{item.name}</p>
                  <div className={style.indexer__tabs__item__images}>
                      <img {...item.images[0]} alt={item.name}/>
                      <img {...item.images[1]} alt={item.name}/>
                  </div>
              </div>
            )
        })
            ...
            return rightItem.items?.map((target, key) =>
              <div className={style.indexer__block__inner__item} key={key}>
                  <div className={style.indexer__block__inner__item_img}>
                      <img key={key} {...target.image} alt={target.title}/>
                  </div>
                  <p>{target.title}</p>
              </div>
            )
        ...
        const setImages = () => {
            return leftItem.images?.map((target, key) =>
              <div className={style.indexer__block__inner__images_item}>
                  <img key={key} {...target} alt={leftItem.title}/>
              </div>
            )
        }
        return (
              ...
              {leftItem.image && (
                <>
                    {leftItem.image.type === 'logo' && (
                      <div className={classNames(style.indexer__block__inner__image)}><img {...leftItem.image.src}
                                                                                           alt={leftItem.title}/></div>
                    )}
                    {leftItem.image.type === 'img' && (
                      <img {...leftItem.image.src} alt={leftItem.title}/>
                    )}
                </>
              )}
          </>
        ...
        return indexerData.diagrams.map((item, index) => {
            return (
              <div key={index} className={style.indexer__block__inner}>
                  <div className={style.indexer__block__inner__wrapper}
                       style={{transform: `translate(-${(activeMode === 'With' ? 0 : 1) * 100}%)`}}>
                      <div className={style.indexer__block__inner__with}>
                          ...
                          <div className={style.indexer__block__inner__center}>
                              {windowWidth < 767.999 ? (
                                <img {...background_with_mobile} className={style.indexer__block__inner__center_arrow}
                                     alt={item.with.center.title}/>
                              ) : (
                                <img {...background_with} className={style.indexer__block__inner__center_arrow}
                                     alt={item.with.center.title}/>
                              )}
                              <div className={style.indexer__block__inner__center__wrapper}>
                                  <img {...item.with.center.image} alt={item.with.center.title}/>
                      ...
                      <div className={style.indexer__block__inner__without}>
                          ...
                          <div className={style.indexer__block__inner__center}>
                              {windowWidth < 767.999 ? (
                                <img {...background_without_mobile} className={style.indexer__block__inner__center_arrow}
                                     alt={item.with.center.title}/>
                              ) : (
                                <img {...background_without} className={style.indexer__block__inner__center_arrow}
                                     alt={item.with.center.title}/>
                              )}
                              <div className={style.indexer__block__inner__center__wrapper}>
                                  <img {...item.without.center.image} alt={item.without.center.title}/>
                                  <p>{item.without.center.title}</p>
                              </div>
                          </div>
                          <div
                            className={classNames(style.indexer__block__inner__right, !item.without.rightBlock.items && !item.without.rightBlock.image && style.indexer__block__inner__right_center)}>
          ...
          <div className={style.indexer}>
              <div className={classNames(style.indexer__container, 'container')}>
                  ...
                  <div className={style.indexer__wrapper}>
                      ...
                      <div className={style.indexer__block} ref={blockRef} onTouchStart={(e) => touchstartEvent(e)}
                           onTouchEnd={(e) => touchendEvent(e)}>
                          ...
                          <div className={style.indexer__block__wrapper}
                               style={{transform: `translate(-${(activeTab) * 100}%)`}}>{setBlock()}</div>
                      </div>
                  </div>
              </div>
          </div>
